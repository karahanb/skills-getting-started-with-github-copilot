      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants.length > 0) {
          ...
          details.participants.forEach((participant) => {
            participantsHTML += `<li style="display:flex;align-items:center;gap:8px;">
              <span>${participant}</span>
              <button class="delete-participant" data-activity="${name}" data-email="${participant}" title="Remove participant" style="background:none;border:none;cursor:pointer;font-size:16px;">🗑️</button>
            </li>`;
          });
          participantsHTML += `</ul>`;
        } else {
